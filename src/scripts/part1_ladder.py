"""The two checks that killed the depth claim, applied to Part 1.

Result 1.1 says the residual stream adds +0.012 to predictability and +0.035 to
readiness. Both numbers come from a fixed 50-component-per-layer budget and are
quoted with a spread over five split seeds. The depth claim in Part 2 rested on
exactly those two choices and did not survive either of them, so the dissociation
has to go through the same two checks before it is worth writing up.

Three things this does that analysis.py does not.

1. Matched feature sets. labels_gtmargin.pt stores layers {1, 3, 5} and
   labels_waitgain.pt stores {1, 3, 5, 6}, and build_families concatenates
   whatever it finds -- 150 hidden columns for predictability against 200 for
   readiness. The contrast between the two targets is therefore also a contrast
   between two capacities, in favour of the target that wins. This script uses
   the intersection of the layer sets and says which layers it dropped.

2. A dimension ladder. If the increment over the output baseline keeps climbing
   with components on one target and plateaus on the other, the dissociation is
   about how compactly each target is encoded rather than about which one the
   residual stream carries. The rung with no PCA at all settles it.

3. An interval on the dissociation itself. The claim is a difference of
   differences -- the hidden increment on readiness minus the hidden increment on
   predictability -- and it has never had one. Both targets are grouped on the
   same 200 windows and split with the same seed, so the same test windows can be
   resampled in both datasets and the two increments differenced inside each
   draw. That is what the third table reports.

Run one target at a time to keep peak memory down; each run caches its
per-window statistics, and the dissociation table appears once both exist.

    python src/scripts/part1_ladder.py predictability
    python src/scripts/part1_ladder.py readiness
    python src/scripts/part1_ladder.py            # both, then the dissociation
"""
